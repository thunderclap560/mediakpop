...
class MediaController extends BaseController {
	...
	for($i=0; $i<count($arr); $i++){
		$media = Media::find($arr[$i]);
		if($media->user_id == Auth::user()->id || Auth::user()->admin == 1){

			$media_flags = MediaFlag::where('media_id', '=', $arr[$i])->get();
			foreach($media_flags as $media_flag){
				$media_flag->delete();
			}

			$media_likes = MediaLike::where('media_id', '=', $arr[$i])->get();
			foreach($media_likes as $media_like){
				$media_like->delete();
			}

			$comments = Comment::where('media_id', '=', $arr[$i])->get();
